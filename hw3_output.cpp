...
void output::printProductionRule(const int ruleno) {
    std::cout << ruleno + 1 << ": " << rules[ruleno] << std::endl;
}
void output::applyProductionRule(const int ruleno) {
    std::cout << "applying rule number " << ruleno + 1 << ": " << rules[ruleno] << ". captured text is: " << yytext << std::endl;
}
