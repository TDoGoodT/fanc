...
struct _T_Binop : public _T_Exp {
    ...
    enum Binop {_PLUS_, _MINUS_, _MULT_, _DIV_} binopCase;
    struct _T_Exp *r_exp;
    struct _T_Exp *l_exp;
    _T_Binop(struct _T_Exp *r_exp, struct _T_Exp *l_exp, Binop binopCase) : r_exp(r_exp), l_exp(l_exp), binopCase(binopCase), _T_Exp(new _T_Type(get_type(r_exp, l_exp))) {};
